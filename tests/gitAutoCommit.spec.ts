...
describe('Git Auto-Commit Functions', () => {
  ...
  describe('gitInit', () => {
    ...
    it('succeeds even if already initialized', async () => {
      // Initialize once
      gitInit(tempDir);
      // Initialize again
      const result = gitInit(tempDir);
  ...
  describe('getAutoCommitInfo', () => {
    ...
    it('returns no changes for clean repository', async () => {
      spawnSync('git', ['init'], { cwd: tempDir });
    ...
    it('generates appropriate commit message for single file', async () => {
      spawnSync('git', ['init'], { cwd: tempDir });
    ...
    it('generates appropriate commit message for multiple files', async () => {
      spawnSync('git', ['init'], { cwd: tempDir });
    ...
    it('generates message for multiple new files', async () => {
      spawnSync('git', ['init'], { cwd: tempDir });
    ...
    it('generates message for multiple doc files', async () => {
      spawnSync('git', ['init'], { cwd: tempDir });
    ...
    it('uses feat prefix for single file addition', async () => {
      spawnSync('git', ['init'], { cwd: tempDir });
      writeFileSync(join(tempDir, 'feature.test.ts'), 'test content');
  ...
  describe('executeAutoCommit', () => {
    ...
    it('commits staged changes', async () => {
      ...
      const result = await executeAutoCommit(tempDir, 'test commit');
    ...
    it('stages all changes when stageAll is true', async () => {
      writeFileSync(join(tempDir, 'file1.txt'), 'content1');
      writeFileSync(join(tempDir, 'file2.txt'), 'content2');

      const result = await executeAutoCommit(tempDir, 'commit multiple files', true);
    ...
    it('returns error when no changes to commit', async () => {
      ...
      const result = await executeAutoCommit(tempDir, 'empty commit');
    ...
    it('returns the short commit hash', async () => {
      writeFileSync(join(tempDir, 'test.txt'), 'content');

      const result = await executeAutoCommit(tempDir, 'test');
